<?php 
  ...
  include 'admin/db.php';
	...
	else
	{
		...
		$prev = "select * from `post` order by `id` desc limit $id,1";
		$prev=mysqli_query($con,$prev);
		$single=mysqli_fetch_assoc($prev);

	$prevc="select * from `post` ";
	...
	$post_res = mysqli_query($con,$post_select);
		...
		if(isset($_POST['submit'])) {
			...
			$path = "./image/comment/".$image;
			move_uploaded_file($_FILES['image']['tmp_name'], $path);

			$insert = "insert into `comment`(`c_id`,`name`,`email`,`subject`,`comment`,`image`)values('$id','$name','$email','$subject','$comment','$image')";
			mysqli_query($con,$insert);
		}
		
		$comment_select = "select * from `comment` where `c_id`=".$id;
...
<head>
	...
	<style>
	    .first-comment-item 
	    {
	        padding-left: calc(10px + <?php echo ($cnt % 2 == 1) ? '5px' : '0px'; ?>);
	    }
	    .blog-comments
	    {
	...
	<div class="sidebar-menu-container" id="sidebar-menu-container">
						...
						<div class="row">
							<div class="col-md-8">
								<div class="blog-single-item">
									<img src="image/latest_posts/<?php echo @$single['image']; ?>" alt="">
									...
									<div class="prev-btn col-md-6 col-sm-6 col-xs-6">
										<?php if($id>0) {  $ids = $id; ?>
										<a href="single_post.php?id=<?php echo --$ids ?>"><i class="fa fa-angle-left"></i>Previous</a>
									<?php } ?>
									</div>

									<div class="next-btn col-md-6 col-sm-6 col-xs-6">
										<?php if($id<$num_rec-1) { $ids = $id;  ?>
										<a href="single_post.php?id=<?php echo ++$ids ?>">Next<i class="fa fa-angle-right"></i></a>
										<?php  } ?>
									</div>	
								</div>
        ...
        <?php $cnt = 1; ?>
							...
							<?php 
										...
										{
							?>
										<li class="first-comment-item" style="padding: <?php echo ($cnt % 2 == 0) ? '0px 50px' : '10px 0px'; ?>; display: flex;">
											<div>
												<img src="image/comment/<?php echo @$comment['image']; ?>" alt="">
											</div>
											<div>
								...
								<div class="widget-item">
									...
									<ul class="recent-item">
										...
										<li class="recent-post-item">
											<a href="single_post.php?id=<?php echo $id; ?>">
												<img src="image/latest_posts/<?php echo @$post['image']; ?>" alt="">
